...
await apiManager.loadData();
  ...
  renderer.renderMainUser(data.mainUser.name, data.mainUser.picture.large, data.mainUser.location);
  console.log(data.friends);
  renderer.renderFriends(data.friends);
console.log(data.quote);
  renderer.renderQuote(data.quote);
  renderer.renderPokemon(data.pokemon.name, data.pokemon.sprites.front_default);
  renderer.renderAboutMe(data.aboutMe[0]);
}
